...
impl local_sync::SyncActor for SenderActor {
    type Contract = local_sync::contract::TellOnly;
    type Tell = Start;
    type Ask = ();
    type Reply = ();
...
fn usage() -> &'static str {
    "Usage: cargo bench --bench spsc_sync_actor_pair -- [options]\n\
     \n\
     Environment overrides:\n\
       AF_SPSC_SYNC_PAIR_MSGS    Iterations (default: 1000000)\n\
       AF_SPSC_SYNC_PAIR_WARMUP  Warmup iterations (default: 10000)\n\
       AF_SPSC_SYNC_PAIR_CAP     Channel capacity floor (default: 16384)\n\
     \n\
     CLI options:\n\
       --msgs N\n\
       --warmup N\n\
...
fn parse_config() -> Result<Config, String> {
    let mut cfg = Config {
        msgs: parse_u64_env("AF_SPSC_SYNC_PAIR_MSGS", 1_000_000).max(1),
        warmup: parse_u64_env("AF_SPSC_SYNC_PAIR_WARMUP", 10_000),
        cap: parse_usize_env("AF_SPSC_SYNC_PAIR_CAP", 16_384).max(1),
    };
...
fn wait_done(done: &AtomicBool, processed: &AtomicU64, label: &str) {
    let deadline = Instant::now() + Duration::from_secs(5);
    while !done.load(Ordering::Acquire) {
        assert!(
            Instant::now() < deadline,
            "timeout waiting for {label}; processed={}",
...
fn run_once(total: u64, cap: usize) -> Duration {
    ...
    let (sender_ref, sender_handle) = local_sync::spawn_with_opts(
        SenderActor { channel },
        local_sync::SpawnOpts {
            mailbox_capacity: 1,
            ..Default::default()
    ...
    sender_handle.wait_for_startup();

    let start = Instant::now();
    assert!(sender_ref.tell(Start(total)), "sender kickoff should enqueue");
    wait_done(done.as_ref(), processed.as_ref(), "sync receiver completion");
...
fn main() {
    ...
    if cfg.warmup > 0 {
        ...
        let _ = run_once(cfg.warmup, warmup_cap);
    }

    let cap = effective_cap(cfg, cfg.msgs);
    let elapsed = run_once(cfg.msgs, cap);
    println!("bench=spsc_sync_actor_pair");
